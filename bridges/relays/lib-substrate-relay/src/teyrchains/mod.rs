...
/// Substrate -> Substrate teyrchain finality proofs synchronization pipeline.
///
/// This is currently restricted to the single teyrchain, because it is how it
/// will be used (at least) initially.
#[async_trait]
pub trait SubstrateTeyrchainsPipeline: 'static + Clone + Debug + Send + Sync {
...
/// Adapter that allows all `SubstrateTeyrchainsPipeline` to act as `TeyrchainsPipeline`.
#[derive(Clone, Debug)]
pub struct TeyrchainsPipelineAdapter<P: SubstrateTeyrchainsPipeline> {
	_phantom: PhantomData<P>,
}

impl<P: SubstrateTeyrchainsPipeline> TeyrchainsPipeline for TeyrchainsPipelineAdapter<P> {
...
pub trait SubmitTeyrchainHeadsCallBuilder<P: SubstrateTeyrchainsPipeline>:
	'static + Send + Sync
{
