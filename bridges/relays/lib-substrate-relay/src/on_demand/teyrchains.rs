...
#[derive(Clone)]
pub struct OnDemandTeyrchainsRelay<P: SubstrateTeyrchainsPipeline, SourceRelayClnt, TargetClnt> {
	/// Relay task name.
...
#[async_trait]
impl<P: SubstrateTeyrchainsPipeline, SourceRelayClnt, TargetClnt>
	...
	TargetClnt: Client<P::TargetChain>,
{
	...
	async fn prove_header(
	...
	) -> Result<(HeaderIdOf<P::SourceTeyrchain>, Vec<CallOf<P::TargetChain>>), SubstrateError> {
		...
		if proved_relay_block != selected_relay_block {
			proved_teyrchain_block = teyrchains_source
				...
				.ok_or_else(|| {
					SubstrateError::MissingRequiredTeyrchainHead(
						para_id,
...
async fn background_task<P: SubstrateTeyrchainsPipeline>(
...
async fn read_relay_data<P: SubstrateTeyrchainsPipeline, SourceRelayClnt, TargetClnt>(
...
where
	...
	TeyrchainsTarget<P, SourceRelayClnt, TargetClnt>:
		TargetClient<TeyrchainsPipelineAdapter<P>> + RelayClient<Error = SubstrateError>,
{
...
#[async_trait]
impl<'a, P: SubstrateTeyrchainsPipeline, SourceRelayClnt, TargetClnt>
	...
	TargetClnt: Client<P::TargetChain>,
{
	...
	) -> Result<Option<HeaderIdOf<P::SourceTeyrchain>>, SubstrateError> {
	...
	PBN: BlockNumberBase,
{
	...
	let best_possible_teyrchain_block = env
		...
		.ok_or(SubstrateError::MissingRequiredTeyrchainHead(
			env.teyrchain_id(),
...
mod tests {
	...
	async fn select_headers_to_prove_returns_err_if_required_para_block_is_missing_at_source() {
		assert!(matches!(
			...
			Err(SubstrateError::MissingRequiredTeyrchainHead(ParaId(0), 300_u64)),
		));
	}
