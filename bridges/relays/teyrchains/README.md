...
runtime. The target chain must have the [bridge teyrchains pallet](../../modules/teyrchains/) deployed at its runtime.
...
- `pezkuwichain_to_BridgeHubzagros_Teyrchains_best_teyrchain_block_number_at_source` - returns best known teyrchain block
  number, registered in the `paras` pallet at the source relay chain (pezkuwichain in our example);

- `pezkuwichain_to_BridgeHubzagros_Teyrchains_best_teyrchain_block_number_at_target` - returns best known teyrchain block
  number, registered in the bridge teyrchains pallet at the target chain (BridgeHubzagros in our example).
