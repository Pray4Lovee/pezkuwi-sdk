...
pub mod pallet {
	...
	/// Weight info of the given teyrchains pallet.
	...
	pub enum Event<T: Config<I>, I: 'static = ()> {
		/// The caller has provided head of teyrchain that the pallet is not configured to track.
		UntrackedTeyrchainRejected {
			/// Identifier of the teyrchain that is not tracked by the pallet.
			teyrchain: ParaId,
		},
		...
		/// The caller has provided obsolete teyrchain head, which is already known to the pallet.
		...
		BoundedBridgeGrandpaConfig<Self::BridgesGrandpaPalletInstance>
	{
		...
		/// we're interested in.
		///
		/// The associated GRANDPA pallet is also used to configure free teyrchain heads
	...
	/// State roots of teyrchain heads which have been imported into the pallet.
	#[pallet::storage]
	...
	impl<T: Config<I>, I: 'static> Pallet<T, I> {
		...
		/// `pezkuwi-runtime-teyrchains::paras` pallet instance, deployed at the bridged chain.
		...
		#[pallet::call_index(0)]
		#[pallet::weight(WeightInfoOf::<T, I>::submit_teyrchain_heads_weight(
			T::DbWeight::get(),
			...
			for (teyrchain, teyrchain_head_hash) in teyrchains {
				...
				let update_result: Result<_, ()> =
					ParasInfo::<T, I>::try_mutate(teyrchain, |stored_best_head| {
						...
						let artifacts = Pallet::<T, I>::update_teyrchain_head(
							teyrchain,
	...
	/// Returns maximal number of teyrchains, supported by the pallet.
	...
	/// Returns total number of all teyrchains hashes/heads, stored by the pallet.
	...
	for TeyrchainHeaders<T, I, C>
{
	fn finalized_header_state_root(hash: HashOf<C>) -> Option<HashOf<C>> {
		Pallet::<T, I>::teyrchain_head(ParaId(C::TEYRCHAIN_ID), hash)
...
pub(crate) mod tests {
	...
	fn import_teyrchain_1_head(
		...
		Pallet::<TestRuntime>::submit_teyrchain_heads(
			RuntimeOrigin::signed(1),
	...
	fn submit_teyrchain_heads_checks_operating_mode() {
		...
		run_test(|| {
			...
			// `submit_teyrchain_heads()` should fail when the pallet is halted.
			...
			// `submit_teyrchain_heads()` should succeed now that the pallet is resumed.
			PalletOperatingMode::<TestRuntime>::put(BasicOperatingMode::Normal);
			assert_ok!(Pallet::<TestRuntime>::submit_teyrchain_heads(
				RuntimeOrigin::signed(1),
				(0, test_relay_header(0, state_root).hash()),
				teyrchains,
				proof,
	...
	fn imports_initial_teyrchain_heads() {
		...
		run_test(|| {
			...
			let result = Pallet::<TestRuntime>::submit_teyrchain_heads(
				RuntimeOrigin::signed(1),
				(0, test_relay_header(0, state_root).hash()),
				teyrchains,
				proof,
	...
	fn is_not_rewriting_existing_head_if_failed_to_read_updated_head() {
		...
		run_test(|| {
			...
			assert_eq!(
				Pallet::<TestRuntime>::best_teyrchain_head(ParaId(1)),
				Some(stored_head_data(1, 5))
			);
