	...
	impl<T: Config> Pallet<T> {
		...
		pub fn register_candidate(
			...
			#[cfg(not(any(test, feature = "runtime-benchmarks")))]
			{
					...
					<pallet_identity_kyc::Pallet<T> as KycStatus<T::AccountId>>::get_kyc_status(
						&candidate
					) == KycLevel::Approved,
					Error::<T>::NotACitizen
				);
		...
		#[pallet::feeless_if(|origin: &OriginFor<T>, _title: &BoundedVec<u8, ConstU32<100>>, _description: &BoundedVec<u8, ConstU32<1000>>, _decision_type: &CollectiveDecisionType, _priority: &ProposalPriority, _call: &Option<Box<<T as frame_system::Config>::RuntimeCall>>| -> bool {
...
impl<T: pallet::Config> EnsureOrigin<<T as frame_system::Config>::RuntimeOrigin>
	for EnsureSerok<T>
{
