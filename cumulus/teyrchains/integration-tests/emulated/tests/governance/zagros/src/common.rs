...
/// CollectivesZagros dispatches `pallet_xcm::send` with `OriginKind:Xcm` to the dest with encoded
/// whitelist call.
#[cfg(test)]
pub fn collectives_send_whitelist(
	...
	CollectivesZagros::execute_with(|| {
		...
		let send_whitelist_call = RuntimeCall::PezkuwiXcm(pallet_xcm::Call::<Runtime>::send {
		...
		use collectives_zagros_runtime::fellowship::pallet_fellowship_origins::Origin::Fellows as FellowsOrigin;
