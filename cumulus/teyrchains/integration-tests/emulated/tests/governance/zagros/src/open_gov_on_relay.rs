...
fn relaychain_can_authorize_upgrade_for_itself() {
	...
	let authorize_upgrade =
		ZagrosRuntimeCall::Utility(pallet_utility::Call::<ZagrosRuntime>::force_batch {
			calls: vec![
				// upgrade the relaychain
				ZagrosRuntimeCall::System(frame_system::Call::authorize_upgrade { code_hash }),
			],
		});
	...
	collectives_send_whitelist(Location::parent(), || {
		ZagrosRuntimeCall::Whitelist(pallet_whitelist::Call::<ZagrosRuntime>::whitelist_call {
			call_hash,
		})
		.encode()
	});
