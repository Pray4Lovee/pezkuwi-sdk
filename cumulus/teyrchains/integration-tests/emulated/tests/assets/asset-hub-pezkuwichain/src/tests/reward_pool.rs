...
fn treasury_creates_asset_reward_pool() {
	...
	Pezkuwichain::execute_with(|| {
		...
		let create_pool_call =
			PezkuwichainRuntimeCall::XcmPallet(pallet_xcm::Call::<PezkuwichainRuntime>::send {
				...
				message: bx!(VersionedXcm::V5(Xcm(vec![
					...
					Transact {
						...
						call: AssetHubPezkuwichainRuntimeCall::AssetRewards(
							pallet_asset_rewards::Call::<AssetHubPezkuwichainRuntime>::create_pool {
								staked_asset_id,
								reward_asset_id,
		...
		assert_expected_events!(
			Pezkuwichain,
			vec![
				PezkuwichainRuntimeEvent::XcmPallet(pallet_xcm::Event::Sent { .. }) => {},
			]
		);
	});
