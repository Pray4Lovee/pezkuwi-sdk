...
fn treasury_creates_asset_reward_pool() {
	...
	Zagros::execute_with(|| {
		...
		let create_pool_call =
			ZagrosRuntimeCall::XcmPallet(pallet_xcm::Call::<ZagrosRuntime>::send {
				...
				message: bx!(VersionedXcm::V5(Xcm(vec![
					...
					Transact {
						...
						call: AssetHubZagrosRuntimeCall::AssetRewards(
							pallet_asset_rewards::Call::<AssetHubZagrosRuntime>::create_pool {
								staked_asset_id,
								reward_asset_id,
		...
		assert_expected_events!(
			Zagros,
			vec![
				ZagrosRuntimeEvent::XcmPallet(pallet_xcm::Event::Sent { .. }) => {},
			]
		);
	});
