...
To convert a Substrate runtime into a Teyrchain runtime, the following code needs to be added to the
runtime:
```rust
...
to a validator that is assigned to the particular Teyrchain. Cumulus provides the block production
...
Teyrchain block for inclusion. The Cumulus runtime pallet will provide the functionality to
...
function provided by Cumulus. So, updating a Teyrchain runtime on the relay chain involves a
