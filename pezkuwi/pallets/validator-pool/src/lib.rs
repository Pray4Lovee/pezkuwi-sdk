...
pub mod pallet {
    ...
    pub enum Event<T: Config> {
        ...
        EraAnalysisRecorded {
            era_index: u32,
            tnpos_total_stake: u128,
            npos_total_stake: u128,
        },
    ...
    impl<T: Config> Pallet<T> {
        ...
        pub fn record_npos_validators(
            ...
            Self::deposit_event(Event::NposValidatorsRecorded {
                era_index,
                validator_count,
            });
    ...
    impl<T: Config> Pallet<T> {
        ...
        fn do_shadow_comparison(
            ...
            CumulativeShadowStats::<T>::mutate(|stats| {
                ...
                let total_selections = stats.total_overlap + stats.total_tnpos_only + stats.total_npos_only;
        ...
        fn calculate_avg_trust(validators: &Vec<T::AccountId>) -> u32 {
            ...
            let total_trust: u128 = validators
                .iter()
                .map(|v| T::TrustSource::trust_score_of(v))
                .sum();
        ...
        fn update_cumulative_stats(era_index: u32, blocks_produced: u32) {
            if let Some(era_analysis) = Self::era_analysis(era_index) {
                CumulativeShadowStats::<T>::mutate(|stats| {
                    stats.npos_actual_blocks = stats.npos_actual_blocks.saturating_add(blocks_produced as u64);
                    ...
                    stats.tnpos_projected_blocks = stats.tnpos_projected_blocks.saturating_add(projected);
        ...
        fn project_tnpos_blocks(_era_index: u32) -> u64 {
            if let Some(tnpos_set) = Self::shadow_validator_set() {
                ...
                for validator in validators {
                    ...
                    if total_blocks > 0 {
                        let success_rate = (performance.blocks_produced as u64 * 100) / total_blocks as u64;
