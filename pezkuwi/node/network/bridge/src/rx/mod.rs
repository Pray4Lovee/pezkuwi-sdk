	...
	N::IntoIter: std::iter::ExactSizeIterator,
{
	...
	for (discovery_id, validator_index) in neighbors {
		...
		if addr.is_none() {
			// See on why is not good in https://github.com/pezkuwichain/pezkuwichain-sdk/issues/2138
			gum::debug!(
				target: LOG_TARGET,
				?validator_index,
				"Could not determine peer_id for validator, let the team know in \n
				https://github.com/pezkuwichain/pezkuwichain-sdk/issues/2138"
			)
		}
		...
		($event:expr, $message:ident) => {
			if let Ok(event) = $event.focus() {
				let has_high_priority = matches!(
					...
					// See https://github.com/pezkuwichain/pezkuwichain-sdk/issues/824
