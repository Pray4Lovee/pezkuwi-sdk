...
/// let myteyrchain: Chain = "myteyrchain".try_into().unwrap();
///
/// assert_eq!(polkadot.as_str(), "polkadot");
/// assert_eq!(kusama.as_str(), "kusama");
/// assert_eq!(myteyrchain.as_str(), "myteyrchain");
/// ```
...
impl de::Visitor<'_> for ArgVisitor {
    ...
    where
        E: de::Error,
    {
        // covers the "-lruntime=debug,teyrchain=trace" case
